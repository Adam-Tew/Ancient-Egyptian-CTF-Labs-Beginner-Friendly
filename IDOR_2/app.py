...
def generate_papyrus_pdf(doc_id, collection="public", regenerate=False):
    ...
    local_seed = int(hashlib.md5(f"egyptian_papyrus_lab_{doc_id}_{collection}".encode()).hexdigest(), 16) % 10000
    ...
    # Set basic metadata for all documents
    ...
    # REAL FLAG: Only in metadata of document #23 in restricted collection
    if collection == "restricted" and doc_id == 23:
        # Put the real flag in the Author field
        pdf.set_author(FLAG)
        print(f"Added real flag to metadata of document {doc_id}")
    else:
        pdf.set_author("Digital Papyrus Project")
    
    # DECOY FLAGS: Add them as visible text in specific documents
    if collection == "restricted" and doc_id in DECOY_DOCS:
        ...
        # Make the decoy flag clearly visible in the PDF text
        pdf.set_font("Times", "", 10)
        pdf.set_text_color(0, 0, 0)
        
        # Position it somewhere on the page
        pdf.set_y(rand_gen.randint(180, 250))
        pdf.set_x(20)
        
        # Add a prefix to make it look like part of the document
        ...
        # Add the decoy flag to the PDF content
        pdf.cell(0, 10, f"{prefix}{decoy_flag}", 0, 1, "L")
        print(f"Added decoy flag to text of document {doc_id}")
