...
def create_virtual_filesystem(session_id):
    ...
    fs_challenge1 = {
        "etc": {
            "passwd": f"Winning number: {winning_number_1}"
        }
    }
    
    fs_challenge2 = {
        "etc": {
            "shadow": f"Next number: {winning_number_2}"
        }
    }
    
    fs_challenge3 = {
        "etc": {
            "hosts": f"Password: {treasure_password}"
        }
    }
...
def submit_guess():
    ...
    try:
        ...
        if guess_num == virtual_filesystems[session['session_id']]["winning_number_1"]:
            ...
            # Use the current request's port instead of hardcoded 5002
            server_name = request.host
            port = request.host.split(':')[1] if ':' in request.host else '5012'  # Default to 5012 if no port
            redirect_url = f"http://{server_name.split(':')[0]}:{port}/{challenge_2_route}/challenge_2"
...
def challenge_2_submit():
    ...
    try:
        ...
        if guess_num == virtual_filesystems[session['session_id']]["winning_number_2"]:
            ...
            server_name = request.host
            port = request.host.split(':')[1] if ':' in request.host else '5012'  # Default to 5012 if no port
            redirect_url = f"http://{server_name.split(':')[0]}:{port}/{challenge_3_route}/challenge_3"
            return redirect(redirect_url)
        else:
